...
def login():
    ...
    if failed_sign_in:
        failed_sign_in = False
        return render_template("login.html", activePage=5, sessionIsTrue="name" in session,
                               failed_sign_in=[rf"Не удалось войти, вы можете зарегистрироваться ", "здесь"])
...
def registration():
    ...
    if request.method == 'POST':
        ...
        # Проверка, что пользователь существует, прежде чем работать с ним
        if user:
            print("Регистрация " + user.name)  # Выводит имя пользователя
            db.session.add(user)
            db.session.commit()
        else:
